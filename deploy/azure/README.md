...
- The **App Service** itself (`appServiceName`, default `scaletrigger-<random>` — a `uniqueString()` suffix derived from the subscription and resource group, since it becomes `<name>.azurewebsites.net` and a bare `scaletrigger` is essentially guaranteed to already be taken by someone else's App Service; override it with your own name if you'd rather have one), with GitHub source control pointed at this repo's `master` branch (`isManualIntegration: true` — works against a public repo with no token or publish profile). App Service's own Oryx build picks up `ScaleTrigger.sln` and builds it on first push, so the site is live shortly after the deployment finishes, no separate CI step needed.
